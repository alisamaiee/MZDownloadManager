...
open class MZDownloadManager: NSObject {
    ...
    ///URL : DownloadModel
    open var downloadingURLsMap = [String: MZDownloadModel]()
    ...
    fileprivate func populateOtherDownloadTasks() {
        ...
        for downloadTask in downloadTasks {
            ...
            if downloadTask.state == .running {
                ...
                downloadingURLsMap[downloadModel.fileURL] = downloadModel
...
extension MZDownloadManager: URLSessionDownloadDelegate {
    ...
    public func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didWriteData bytesWritten: Int64, totalBytesWritten: Int64, totalBytesExpectedToWrite: Int64) {
        for downloadModel in self.downloadingURLsMap.values {
            if downloadTask.isEqual(downloadModel.task) {
                DispatchQueue.main.async(execute: { () -> Void in
                    ...
                    if self.downloadingURLsMap[downloadModel.fileURL] != nil {
                        self.downloadingURLsMap[downloadModel.fileURL] = downloadModel
                    }
    ...
    public func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        DispatchQueue.main.async {
            ...
            if (err?.userInfo[NSURLErrorBackgroundTaskCancelledReasonKey] as? NSNumber)?.intValue == NSURLErrorCancelledReasonUserForceQuitApplication || (err?.userInfo[NSURLErrorBackgroundTaskCancelledReasonKey] as? NSNumber)?.intValue == NSURLErrorCancelledReasonBackgroundUpdatesDisabled {
                ...
                self.delegate?.downloadRequestDidPopulatedInterruptedTasks(Array(self.downloadingURLsMap.values))
                
            } else {
                //for(index, object) in self.downloadingArray.enumerated() {
                for object in self.downloadingURLsMap.values {
                    let downloadModel = object
                    if task.isEqual(downloadModel.task) {
                        if err?.code == NSURLErrorCancelled || err == nil {
                            self.downloadingURLsMap.removeValue(forKey: downloadModel.fileURL)
...
extension MZDownloadManager {
    ...
    @objc public func pauseDownloadTaskWithURL(_ url: String) {
        guard let downloadModel = downloadingURLsMap[url] else { return }
        ...
        downloadingURLsMap[url] = downloadModel
...
//    }
    ...
    @objc public func resumeDownloadTaskWithURL(_ url: String) {
        guard let downloadModel = downloadingURLsMap[url] else { return }
    ...
    @objc public func retryDownloadTaskWithURL(_ url: String) {
        guard let downloadModel = downloadingURLsMap[url] else { return }
    ...
    @objc public func cancelTaskWithURL(_ url: String) {
        guard let downloadModel = downloadingURLsMap[url] else { return }
        let downloadTask = downloadModel.task
        downloadTask!.cancel()
    }
